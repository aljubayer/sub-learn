...
/**
 * Load Codexin Framework Configuration.
 *
 */
...
gulp.task('materializeCSS', () => {
    return gulp
        .src(config.styleMaterializeSRC, { allowEmpty: true })
        ...
        .pipe(gulp.dest(config.styleMaterializeDestination))
 ...
 *    2. Compiles Sass to CSS
 *    3. Writes Sourcemaps for it
 *    4. Autoprefixes it and generates shortcodes.css
 */
gulp.task('shortcodes', () => {
    return gulp
        .src(config.styleShortcodeSRC, { allowEmpty: true })
        .pipe(plumber(errorHandler))
        .pipe(sourcemaps.init())
        .pipe(
            sass({
...
gulp.task(
    'run',
    gulp.parallel('styles','materializeCSS', 'shortcodes', 'vendorsJS', 'customJS', 'images', browsersync, () => {
        gulp.watch(config.watchPhp, reload); // Reload on PHP file changes.
        gulp.watch(config.watchStyles, gulp.parallel('materializeCSS')); // Reload on SCSS file changes.
        gulp.watch(config.watchStyles, gulp.parallel('styles')); // Reload on SCSS file changes.
        gulp.watch(config.watchStyles, gulp.parallel('shortcodes')); // Reload on SCSS file changes.
...
gulp.task(
	'build', 
	gulp.series('clean', 'buildStyles', 'buildShortcodes', 'vendorsJS', 'customJS', 'buildFiles', 'buildImages', 'buildZip', function (done) {
	    done();
	})
);
